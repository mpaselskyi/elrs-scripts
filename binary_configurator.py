...
def patch_firmware(options, mm, pos, args):
    if options.mcuType is MCUType.STM32:
        if options.radioChip is RadioType.SX127X and args.idomain:
            mm[pos] = args.idomain
        pos += 1
...
def patch_unified(args, options):
    ...
    elif not args.no_auto_wifi:
        json_flags['wifi-on-interval'] = 60
    ...
    if args.idomain is not None:
        json_flags['domain'] = args.idomain
...
def main():
    ...
    parser.add_argument('--idomain', type=int, default=None, help='For SX127X based devices, which regulatory domain is being used')
    # Unified target
    ...
    if (args.idomain is None and args.domain is not None):
        args.idomain = domain_number(args.domain)
